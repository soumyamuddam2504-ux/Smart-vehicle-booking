    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="relative h-72 md:h-80 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-4">
          <span className="text-xs font-semibold uppercase tracking-widest text-blue-300 mb-3">Smart Vehicle Matching</span>
      ...
      <div className="max-w-5xl mx-auto px-4 pb-12">
        {/* Search Form — floats over the hero */}
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 md:p-8 -mt-10 relative z-10">
          ...
          <form onSubmit={handleSearch}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Total Budget ($)</label>
                <input
                  type="number"
                  name="budget"
                  value={form.budget}
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">No. of Seats</label>
                <input
                  type="number"
                  name="passengers"
            ...
            {/* Row 2: Trip Type + Duration */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Trip Type</label>
                <select
                  name="tripType"
                  value={form.tripType}
                  onChange={handleChange}
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
                  Duration (days)
                  {durationDays > 0 && (
                    <span className="ml-2 text-blue-600 normal-case font-medium">auto-calculated</span>
                  )}
                </label>
                <input
                  type="number"
        ...
        {/* Alerts */}
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 text-sm rounded-xl px-4 py-3 mt-6">{error}</div>
        )}
        {message && (
          <div className="bg-amber-50 border border-amber-200 text-amber-800 text-sm rounded-xl px-4 py-3 mt-6 flex items-start gap-2">
            <span className="text-amber-500 mt-0.5">ⓘ</span> {message}
          </div>
        )}
        ...
        {results && results.length === 0 && !message && (
          <div className="mt-10 bg-white rounded-2xl border border-gray-100 shadow-md text-center py-16 px-8">
            <p className="text-4xl mb-4">🔍</p>
            <p className="text-gray-800 font-semibold text-lg">No RVs found</p>
            <p className="text-gray-400 text-sm mt-1">Try adjusting your budget or reducing the number of seats.</p>
          </div>
        )}
      </div>
    </div>
